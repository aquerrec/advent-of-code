 ...
 */
data object Day11Solver : AdventOfCodeSolver<Sequence<String>, Long> {
    val START_DEVICE = Vertex("you")
    val END_DEVICE = Vertex("out")
    val SERVER_RACK = Vertex("svr")
    val DAC_DEVICE = Vertex("dac")
    val FFT_DEVICE = Vertex("fft")

    override fun solvePart1(input: Sequence<String>): Long = input.toGraph().numberOfPaths(START_DEVICE, END_DEVICE)

    override fun solvePart2(input: Sequence<String>): Long =
        input.toGraph().let { graph ->
            graph.numberOfPaths(listOf(SERVER_RACK, DAC_DEVICE, FFT_DEVICE, END_DEVICE)) +
                graph.numberOfPaths(listOf(SERVER_RACK, FFT_DEVICE, DAC_DEVICE, END_DEVICE))
        }

    private fun <T> Graph<T>.numberOfPaths(vertices: List<Vertex<T>>) =
        vertices
            .zipWithNext { src, dest -> this.numberOfPaths(src, dest) }
            .fold(1L) { counter, i -> counter * i }

    private fun Sequence<String>.toGraph(): Graph<String> =
        this
            .map { line -> line.split(": ") }
            .associate { (device, connections) -> device to connections.split(" ") }
            .let { Graph.fromDirectedGraph(it) }
}
