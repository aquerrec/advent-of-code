...
import utils.dropFirstAndLast
import utils.splitToInt
 ...
 */
data object Day10Solver : AdventOfCodeSolver<Sequence<String>, Long> {
    override fun solvePart1(input: Sequence<String>): Long = input.parseMachines().sumOf { it.fewestButtonPressesRequired() }

    override fun solvePart2(input: Sequence<String>) = 1L // TODO
    ...
    data class Machine(
        ...
        val lightsBitmask =
            lights
                ...
                .reversed()
                .toLong(2)
        val wiringSchematicsBitmasks = wiringSchematics.map { it.fold(0L) { mask, i -> mask or (1L shl i) } }

        fun fewestButtonPressesRequired(): Long {
            // DFS algorithm
            val queue = ArrayDeque<Pair<Long, Long>>().apply { add(0L to 0L) }
            val visited = mutableMapOf(0L to 0L)
            var minButtonPresses = Long.MAX_VALUE

            while (queue.isNotEmpty()) {
                val (mask, buttonPresses) = queue.removeFirst()
                when {
                    mask == lightsBitmask -> {
                        minButtonPresses = buttonPresses.coerceAtMost(minButtonPresses)
                        continue
                    }

                    buttonPresses >= minButtonPresses || buttonPresses >= wiringSchematics.size -> {
                        continue
                    }

                    else -> {
                        wiringSchematicsBitmasks.forEach { buttonMask ->
                            val newMask = mask xor buttonMask // toggle lights
                            val newButtonPresses = buttonPresses + 1

                            if (newMask !in visited || visited.getValue(newMask) > newButtonPresses) {
                                visited[newMask] = newButtonPresses
                                queue.addLast(newMask to newButtonPresses)
                            }
                        }
                    }
                }
    ...
    private fun Sequence<String>.parseMachines(): Sequence<Machine> =
        this.map { line ->
            val tokens = line.split(" ")

            val lights = tokens.first().dropFirstAndLast(1)
            val wiringSchematics = tokens.drop(1).dropLast(1).map { it.dropFirstAndLast(1).splitToInt(",") }
            val joltageRequirements = tokens.last().dropFirstAndLast(1).splitToInt(",")

            Machine(lights, wiringSchematics, joltageRequirements)
        }
}
