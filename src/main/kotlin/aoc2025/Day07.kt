 ...
 */
data object Day07Solver : AdventOfCodeSolver<Matrix<Char>, Long> {
    const val START_CHAR = 'S'
    const val SPLITTER_CHAR = '^'

    override fun solvePart1(input: Matrix<Char>): Long =
        countBeamSplits(
            matrix = input,
            points = listOf(input.beamEntryPoint()),
        )

    override fun solvePart2(input: Matrix<Char>): Long =
        countTimelines(
            matrix = input,
            start = input.beamEntryPoint(),
            visited = mutableMapOf(),
        ) + 1

    private fun Matrix<Char>.beamEntryPoint(): Point = this.first { it == START_CHAR }

    fun countBeamSplits(
        matrix: Matrix<Char>,
        points: List<Point>,
    ): Long {
        val validPoints = points.filter { it in matrix }

        if (validPoints.isEmpty()) return 0L
            ...
            validPoints
                .flatMap {
                    when (matrix[it]) {
                        SPLITTER_CHAR -> {
                            listOf(
                                it.move(Direction.DOWN_LEFT),
                                it.move(Direction.DOWN_RIGHT),
                            )
                        }

                        else -> {
                            listOf(it.move(Direction.DOWN))
                        }
                    }
                }.distinct()

        return validPoints.count { matrix[it] == SPLITTER_CHAR }.toLong() +
            countBeamSplits(matrix, nextPoints)
    }

    private fun countTimelines(
        matrix: Matrix<Char>,
        start: Point,
        visited: MutableMap<Point, Long>,
    ): Long =
        visited.getOrPut(start) {
            if (start !in matrix) {
                0L
            } else if (matrix[start] == SPLITTER_CHAR) {
                1L + countTimelines(matrix, start.move(Direction.DOWN_LEFT), visited) +
                    countTimelines(matrix, start.move(Direction.DOWN_RIGHT), visited)
            } else {
                countTimelines(matrix, start.move(Direction.DOWN), visited)
            }
        }
}
